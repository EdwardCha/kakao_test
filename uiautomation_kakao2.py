...
# ================= 사용자 설정 =================
CHATROOM_NAME = "차희상"
MESSAGE_TEXT  = "안녕하세요, 파이썬 자동 메시지입니다."
DEBUG = True
...
def open_chatroom(name):
    ...
    if not found:
        _log("[open_chatroom] 대상 못 찾음 → 최상단 입장 시도")
        PostKeyEx(list_hwnd, win32con.VK_HOME, [])
        time.sleep(0.05)
        PostKeyEx(list_hwnd, win32con.VK_RETURN, [])
        time.sleep(0.6)
...
def send_message_and_verify(chatroom, text):
    ...
    # 우선순위 1) UIA SendKeys로 실제 타이핑 후, 다양한 Enter 조합 시도
    sent=False
    try:
        ...
        _log(f"[send] typed via UIA, edit_now='{cur_txt}'")
        # Enter 변형 시도 (Ctrl+Enter → Enter)
        if _send_enter_variants(hwndEdit, via_root=root):
            sent=True
            _log("[send] Enter variants 시도 완료")
    except Exception as e:
        _log("[send] UIA SendKeys 타이핑 실패:", e)
    ...
    if not sent:
        try:
            ...
            time.sleep(0.05)
            cur_txt = _get_text(hwndEdit)
            _log(f"[send] pasted, edit_now='{cur_txt}'")
            if _send_enter_variants(hwndEdit, via_root=root):
                sent=True
                _log("[send] WM_PASTE 후 Enter variants 시도 완료")
        except Exception as e:
            _log("[send] WM_PASTE 실패:", e)

    # 3) WM_SETTEXT → Enter (최후 수단)
    if not sent:
        try:
            win32api.SendMessage(hwndEdit, win32con.WM_SETTEXT, 0, text)
            time.sleep(0.05)
            cur_txt = _get_text(hwndEdit)
            _log(f"[send] settext, edit_now='{cur_txt}'")
            if _send_enter_variants(hwndEdit, via_root=root):
                sent=True
                _log("[send] WM_SETTEXT 후 Enter variants 시도 완료")
        except Exception as e:
            _log("[send] WM_SETTEXT 실패:", e)
